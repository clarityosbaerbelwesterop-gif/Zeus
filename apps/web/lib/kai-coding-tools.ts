...
function tool<TInput extends Readonly<Record<string, unknown>>, TOutput>(input: {
  ...
  return {
    ...
    outputContract:
      "Structured, bounded execution evidence. Secrets and raw credentials are never returned.",
...
async function loadBinding(actorId: string, workspaceId: string): Promise<BindingSnapshot> {
  return withActor(actorId, async (db) => {
    ...
    if (!row)
      throw new RuntimeError("CONNECTION_REQUIRED", "No repository is attached to this workspace.");
    return {
      id: row.id,
...
async function resolveRepositoryToken(
...
): Promise<{ token: string; credentials: RepositoryCredentials } | null> {
    ...
    return (
      await db
        .select({
          ...
          secretRef: connections.secretRef,
        })
        .from(connections)
        .where(
          and(
            eq(connections.id, binding.connectionId!),
            eq(connections.workspaceId, binding.workspaceId),
          ),
        )
        .limit(1)
    )[0];
  ...
  if (!connection.secretRef.startsWith("env:")) {
    throw new RuntimeError(
      "CONNECTION_REQUIRED",
      "GitHub credential broker reference is unsupported by this deployment.",
    );
  }
  ...
  if (!token || !expiry)
    throw new RuntimeError(
      "CONNECTION_REQUIRED",
      "Short-lived GitHub credentials are unavailable.",
    );
  ...
  if (
    ...
    throw new RuntimeError(
      "CONNECTION_REQUIRED",
      "GitHub credentials are expired or are not short-lived.",
    );
  }
  return {
    token,
...
async function ensureEnvironment(
...
): Promise<{ binding: BindingSnapshot; environment: ExecutionEnvironment; token: string | null }> {
  ...
  if (head.exitCode !== 0)
    throw new RuntimeError(
      ...
      "Sandbox repository HEAD could not be verified.",
    );
  const currentHeadSha = validateBaseSha(head.stdout.trim());
  await withActor(context.actorId, async (db) => {
    const existing = (
      await db
        .select({ id: executionSessions.id })
        .from(executionSessions)
        .where(eq(executionSessions.runId, context.runId))
        .limit(1)
    )[0];
    if (existing) {
      await db
...
async function assertContained(
  ...
  const result = await provider.execute(environment, {
    command: "node",
    args: ["-e", script, safePath],
  });
  if (result.exitCode !== 0)
    throw new RuntimeError(
      "TOOL_PERMISSION_DENIED",
      "Repository path resolves outside the sandbox worktree.",
    );
  return safePath;
}
...
async function packageManager(
  ...
  for (const [file, name] of [
    ...
    const check = await provider.execute(environment, {
      command: "git",
...
async function executeQualityGate(
  ...
  if (!QUALITY_GATES.has(gate))
    throw new RuntimeError("TOOL_INPUT_INVALID", "Unknown quality gate.");
...
function safeCommandOutput(result: SandboxCommandResult): {
  exitCode: number;
  stdout: string;
  stderr: string;
  truncated: boolean;
  ...
  return {
    ...
    truncated:
      result.truncated || result.stdout.length > MAX_DIFF_BYTES || result.stderr.length > 16_000,
  };
}
...
async function currentBranchAndHead(
  context: ToolExecutionContext,
): Promise<{ branch: string; head: string; binding: BindingSnapshot }> {
  const binding = await loadBinding(context.actorId, context.workspaceId);
  ...
  if (branch.exitCode !== 0 || head.exitCode !== 0)
    throw new RuntimeError("TOOL_EXECUTION_FAILED", "Git branch state could not be verified.");
  const actualBranch = branch.stdout.trim();
  if (actualBranch !== binding.featureBranch)
    throw new RuntimeError(
      "TOOL_PERMISSION_DENIED",
      "Sandbox is not on the attached zeus/* feature branch.",
    );
...
const repositoryToolDefinitions: readonly ToolDefinition[] = [
  tool({
    ...
    description:
      "Pin a workspace to one GitHub repository, exact base commit SHA, and zeus/* feature branch without starting a sandbox.",
    sideEffect: 1,
    parse(value) {
      const input = objectInput(value);
      const repositoryFullName = validateRepositoryFullName(
        requiredString(input, "repositoryFullName", 240),
      );
      ...
      return withActor(context.actorId, async (db) => {
        const existing = (
          await db
            ...
            .where(
              and(
                eq(repositoryBindings.workspaceId, context.workspaceId),
                eq(repositoryBindings.repositoryFullName, input.repositoryFullName),
              ),
            )
            .limit(1)
        )[0];
        ...
        if (!row)
          throw new RuntimeError("TOOL_EXECUTION_FAILED", "Repository binding was not persisted.");
        return {
          bindingId: row.id,
          repositoryFullName: input.repositoryFullName,
          baseSha: input.baseSha,
    ...
    summarizeInput: (input) =>
      `Attach ${input.repositoryFullName} at exact base ${input.baseSha.slice(0, 12)} on ${input.featureBranch}.`,
    summarizeOutput: (output) =>
      `Repository ${output.repositoryFullName} attached at ${output.baseSha.slice(0, 12)} on ${output.featureBranch}.`,
  }),
  tool({
    id: "repo.map",
    name: "Map repository",
  ...
  tool({
    ...
    async execute(input, context) {
      ...
      if (Buffer.byteLength(content, "utf8") > MAX_FILE_BYTES)
        throw new RuntimeError("TOOL_EXECUTION_FAILED", "File exceeds the bounded read limit.");
      return { path: safePath, content };
    },
    summarizeOutput: (output) =>
      `Read ${output.path} (${Buffer.byteLength(output.content, "utf8")} bytes).`,
  }),
  tool({
    id: "repo.search",
    ...
    parse(value) {
      ...
      return {
        ...
        path: optionalString(input, "path", 1_024),
      };
    },
    async execute(input, context) {
  ...
  tool({
    ...
    description:
      "Write one bounded file inside the isolated repository after path and symlink containment checks.",
    sideEffect: 2,
    parse(value) {
      const input = objectInput(value);
      ...
      if (Buffer.byteLength(content, "utf8") > MAX_FILE_BYTES)
        throw new RuntimeError("TOOL_INPUT_INVALID", "File content exceeds the write limit.");
      return { path, content };
    },
    async execute(input, context) {
  ...
  tool({
    ...
    description:
      "Execute a structured command without a shell in the isolated sandbox. Dangerous and supervised network commands are denied.",
    sideEffect: 2,
    parse(value) {
      const input = objectInput(value);
      const command = requiredString(input, "command", 120);
      if (!/^[A-Za-z0-9._+/-]+$/.test(command))
        throw new RuntimeError("TOOL_INPUT_INVALID", "Command contains unsupported characters.");
    ...
    async execute(input, context) {
      ...
      if (!commandAllowed("supervised", risk))
        throw new RuntimeError(
          "TOOL_PERMISSION_DENIED",
          `Command risk ${risk} is not permitted in supervised mode.`,
        );
      return safeCommandOutput(
        await run(context, input.command, input.args, input.cwd ?? undefined),
      );
    },
  }),
  tool({
    ...
    description:
      "Run one canonical test, lint, typecheck, or build script and persist verification evidence.",
    sideEffect: 2,
    parse(value) {
      ...
      if (!QUALITY_GATES.has(gate))
        throw new RuntimeError(
          "TOOL_INPUT_INVALID",
          "gate must be test, lint, typecheck, or build.",
        );
      return { gate };
    },
    async execute(input, context) {
  ...
  tool({
    ...
    async execute(input, context) {
      return safeCommandOutput(
        await run(context, "git", input.staged ? ["diff", "--cached", "--"] : ["diff", "--"]),
      );
    },
  }),
  tool({
    ...
    description:
      "Stage repository changes, scan the staged diff for common secret material, and create a local commit on the zeus/* branch. This never pushes.",
    sideEffect: 3,
    parse(value) {
    ...
    async execute(input, context) {
      ...
      if (stage.exitCode !== 0)
        throw new RuntimeError("TOOL_EXECUTION_FAILED", "Git staging failed.");
      const diff = await run(context, "git", ["diff", "--cached", "--"]);
      if (diff.exitCode !== 0)
        throw new RuntimeError("TOOL_EXECUTION_FAILED", "Staged diff could not be inspected.");
      if (!diff.stdout.trim())
        throw new RuntimeError("TOOL_INPUT_INVALID", "There are no staged changes to commit.");
      if (containsSecret(diff.stdout)) {
        await run(context, "git", ["reset"]);
        throw new RuntimeError(
          "TOOL_PERMISSION_DENIED",
          "Potential secret material detected in the staged diff; commit blocked.",
        );
      }
      const committed = await run(context, "git", ["commit", "-m", input.message]);
      if (committed.exitCode !== 0)
        throw new RuntimeError("TOOL_EXECUTION_FAILED", "Local commit failed.");
      const state = await currentBranchAndHead(context);
      return {
        branch: state.branch,
        headSha: state.head,
        summary: committed.stdout.slice(0, 4_096),
      };
    },
  }),
  tool({
    ...
    description:
      "Persist current HEAD and an optional git stash object without altering the working tree.",
    sideEffect: 1,
    parse(value) {
    ...
    async execute(input, context) {
      const state = await currentBranchAndHead(context);
      const stash = await run(context, "git", [
        "stash",
        "create",
        `zeus checkpoint: ${input.label}`,
      ]);
      if (stash.exitCode !== 0)
        throw new RuntimeError("TOOL_EXECUTION_FAILED", "Checkpoint worktree snapshot failed.");
      ...
      if (!session)
        throw new RuntimeError("TOOL_EXECUTION_FAILED", "Execution session was not persisted.");
      const row = await withActor(
        context.actorId,
        async (db) =>
          (
            await db
              .insert(executionCheckpoints)
              .values({
                organizationId: context.organizationId,
                workspaceId: context.workspaceId,
                runId: context.runId,
      ...
      return {
        checkpointId: row.id,
        headSha: state.head,
        hasWorktreeSnapshot: Boolean(worktreeSha),
      };
    },
  }),
  tool({
    ...
    async execute(input, context) {
      const checkpoint = await withActor(
        context.actorId,
        async (db) =>
          (
            await db
              ...
              .where(
                and(
                  eq(executionCheckpoints.id, input.checkpointId),
                  eq(executionCheckpoints.runId, context.runId),
                ),
              )
              .limit(1)
          )[0],
      ...
      if (!checkpoint)
        throw new RuntimeError("TOOL_INPUT_INVALID", "Checkpoint does not belong to this run.");
      const reset = await run(context, "git", ["reset", "--hard", checkpoint.headSha]);
      if (reset.exitCode !== 0)
        throw new RuntimeError("TOOL_EXECUTION_FAILED", "Checkpoint HEAD restore failed.");
      const clean = await run(context, "git", ["clean", "-fd"]);
      if (clean.exitCode !== 0)
        throw new RuntimeError("TOOL_EXECUTION_FAILED", "Checkpoint clean restore failed.");
      if (checkpoint.worktreeSha) {
        const apply = await run(context, "git", [
          "stash",
          "apply",
          "--index",
          checkpoint.worktreeSha,
        ]);
        if (apply.exitCode !== 0)
          throw new RuntimeError("TOOL_EXECUTION_FAILED", "Checkpoint worktree restore failed.");
      }
      return {
        checkpointId: checkpoint.id,
        restoredHeadSha: checkpoint.headSha,
        restoredWorktree: Boolean(checkpoint.worktreeSha),
      };
    },
  }),
  tool({
    ...
    description:
      "Run all canonical repository quality gates and persist deterministic completion evidence. Kai cannot finish a coding run without this tool.",
    sideEffect: 2,
    timeoutMs: 20 * 60_000,
    parse: () => ({}),
    async execute(_input, context) {
      ...
      for (const gate of ["lint", "typecheck", "test", "build"]) {
        ...
        if (result.exitCode !== 0) {
          throw new RuntimeError(
            "VERIFICATION_FAILED",
            `${gate} failed in the isolated sandbox; repair is required before completion.`,
          );
        }
      }
      const state = await currentBranchAndHead(context);
  ...
  tool({
    ...
    description:
      "Persist an exact push request for user approval. This tool never pushes by itself.",
    sideEffect: 1,
    parse: () => ({}),
    async execute(_input, context) {
      const state = await currentBranchAndHead(context);
      const row = await withActor(
        ...
        async (db) =>
          (
            await db
              .insert(repositoryChangeRequests)
              .values({
                organizationId: context.organizationId,
                workspaceId: context.workspaceId,
                runId: context.runId,
              ...
              .onConflictDoNothing()
              .returning({ id: repositoryChangeRequests.id })
          )[0],
      );
      if (row)
        return {
      ...
      const existing = await withActor(
        ...
        async (db) =>
          (
            await db
              .select({ id: repositoryChangeRequests.id })
              .from(repositoryChangeRequests)
              .where(
                and(
      ...
      if (!existing)
        throw new RuntimeError(
          "TOOL_EXECUTION_FAILED",
          "Push approval request could not be persisted.",
        );
      return {
  ...
  tool({
    ...
    description:
      "Persist an exact pull request request for explicit user approval. This tool never opens a PR by itself.",
    sideEffect: 1,
    parse(value) {
      const input = objectInput(value);
      return {
        title: requiredString(input, "title", 240),
        body: optionalString(input, "body", 12_000) ?? "",
      };
    },
    async execute(input, context) {
      const state = await currentBranchAndHead(context);
      const pushed = await withActor(
        context.actorId,
        async (db) =>
          (
            await db
      ...
      if (!pushed)
        throw new RuntimeError(
          "TOOL_PERMISSION_DENIED",
          "The exact feature branch HEAD must be approved and pushed before preparing a pull request.",
        );
      const row = await withActor(
        context.actorId,
        async (db) =>
      ...
      if (!row)
        throw new RuntimeError(
          "TOOL_EXECUTION_FAILED",
          "Pull request approval request already exists or could not be persisted.",
        );
      return {
        requestId: row.id,
        status: "pending_approval",
        expectedHeadSha: state.head,
        branch: state.branch,
        base: state.binding.defaultBranch,
      };
    },
  }),
];
...
export async function listRunRepositoryChangeRequests(actorId: string, runId: string) {
  return withActor(actorId, async (db) =>
    db
      .select()
      .from(repositoryChangeRequests)
      .where(eq(repositoryChangeRequests.runId, runId))
      .orderBy(repositoryChangeRequests.createdAt),
  );
}
...
async function loadChangeRequest(actorId: string, requestId: string) {
  return withActor(actorId, async (db) => {
    ...
    if (!request)
      throw new RuntimeError("WORKSPACE_ACCESS_DENIED", "Repository change request not found.");
    const binding = (
      await db
        .select()
        .from(repositoryBindings)
        .where(eq(repositoryBindings.id, request.repositoryBindingId))
        .limit(1)
    )[0];
    if (!binding)
      throw new RuntimeError(
        "TOOL_EXECUTION_FAILED",
        "Repository binding for change request is missing.",
      );
    return { request, binding };
  });
}

export async function approveAndExecuteRepositoryChangeRequest(
  actorId: string,
  requestId: string,
): Promise<{ status: string; externalUrl?: string }> {
  const initial = await loadChangeRequest(actorId, requestId);
  if (initial.request.status !== "pending")
    throw new RuntimeError(
      "TOOL_INPUT_INVALID",
      "Repository change request is not pending approval.",
    );
  await withActor(actorId, async (db) => {
    await db
      .update(repositoryChangeRequests)
      .set({
        status: "approved",
        approvedBy: actorId,
        approvedAt: new Date(),
      ...
      .where(
        and(
          eq(repositoryChangeRequests.id, requestId),
          eq(repositoryChangeRequests.status, "pending"),
        ),
      );
  });
  ...
  if (!tokenData)
    throw new RuntimeError(
      "CONNECTION_REQUIRED",
      "Approved GitHub write requires a short-lived connected GitHub credential.",
    );
  await withActor(actorId, async (db) => {
    await db
      .update(repositoryChangeRequests)
  ...
  try {
    if (request.operation === "push") {
        ...
        async (db) =>
          (
            await db
              .select()
              .from(executionSessions)
              .where(eq(executionSessions.runId, request.runId))
              .limit(1)
          )[0],
      );
      if (!session)
        throw new RuntimeError(
          "TOOL_EXECUTION_FAILED",
          "Approved push has no persisted sandbox session.",
        );
      ...
      const provider = new VercelSandboxProvider();
      const head = await provider.execute(environment, {
        command: "git",
        args: ["rev-parse", "HEAD"],
      });
      if (
        head.exitCode !== 0 ||
        head.stdout.trim().toLowerCase() !== request.expectedHeadSha.toLowerCase()
      ) {
        throw new RuntimeError(
          "STALE_RUN",
          "Feature branch HEAD changed after approval request; push blocked.",
        );
      }
      const pushed = await provider.execute(environment, {
        command: "git",
        args: ["push", "origin", `${request.headBranch}:${request.headBranch}`],
        timeoutMs: 10 * 60_000,
      });
      if (pushed.exitCode !== 0)
        throw new RuntimeError("TOOL_EXECUTION_FAILED", "Approved feature branch push failed.");
      await withActor(actorId, async (db) => {
        await db
          .update(repositoryChangeRequests)
    ...
    const response = await fetch(
      `https://api.github.com/repos/${binding.repositoryFullName}/pulls`,
      {
        method: "POST",
        headers: {
    ...
    const safeError =
      error instanceof RuntimeError
        ...
        : "Repository change execution failed.";
    await withActor(actorId, async (db) => {
      await db
        .update(repositoryChangeRequests)
        .set({ status: "failed", safeError: safeError.slice(0, 4_096), updatedAt: new Date() })
        .where(eq(repositoryChangeRequests.id, requestId));
    }).catch(() => undefined);
    throw error;
  }
