...
export function WorkspaceHome({ data }: { data: WorkspacePageData }) {
  ...
  const activeRuns = data.runs.filter((run) => ["queued", "running", "waiting"].includes(run.status));
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1.35fr_.9fr]">
        <section className="soft-panel rounded-[22px] p-5 sm:p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[11px] uppercase tracking-[0.18em] text-[var(--muted)]">Current focus</p>
              <h2 className="mt-1 text-xl font-semibold">
                {workspace.currentFocus || data.activePlan?.title || "Choose the next important thing"}
              </h2>
            </div>
            <Link href={workspaceHref(workspace.id, "plans")} className="text-xs text-[var(--muted)] underline underline-offset-4">
              Plans
            </Link>
          </div>
          {data.activePlan ? (
            <div className="mt-5 border-t border-[var(--line)] pt-4">
              ...
              <p className="mt-1 text-sm leading-6 text-[var(--muted)]">{data.activePlan.objective}</p>
        ...
        <section className="soft-panel rounded-[22px] p-5 sm:p-6">
          <p className="text-[11px] uppercase tracking-[0.18em] text-[var(--muted)]">Needs attention</p>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <section>
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link href={workspaceHref(workspace.id, "tasks")} className="text-xs text-[var(--muted)]">
              View all
            </Link>
          </div>
          <div className="space-y-2">
            {activeTasks.slice(0, 6).map((task) => (
              <div key={task.id} className="flex items-center justify-between gap-4 rounded-2xl border border-[var(--line)] bg-white/35 px-4 py-3">
                ...
                <span className="text-[11px] uppercase tracking-wider text-[var(--muted)]">{task.priority}</span>
              </div>
            ))}
            {!activeTasks.length ? (
              <EmptyState
        ...
        <section>
          <div className="mb-3 flex items-center justify-between">
            ...
            <Link href={workspaceHref(workspace.id, "artifacts")} className="text-xs text-[var(--muted)]">
              View all
            </Link>
          </div>
          <div className="space-y-2">
            {data.artifacts.slice(0, 6).map((artifact) => (
              <div key={artifact.id} className="rounded-2xl border border-[var(--line)] bg-white/35 px-4 py-3">
                <p className="text-sm font-medium">{artifact.title}</p>
                <p className="mt-1 text-xs text-[var(--muted)]">
                  {artifact.creatingAgent ?? "User"} · {artifact.kind} · {timeLabel(artifact.createdAt)}
                </p>
              </div>
            ))}
