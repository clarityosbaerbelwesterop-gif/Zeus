...
test("new user can persist a workspace, conversation, message and waiting run across reload", async ({ page }) => {
  ...
  await page.getByPlaceholder("Give Zeus real work…").fill("Persist this task without faking an AI response.");
  await page.locator("form").filter({ has: page.getByPlaceholder("Give Zeus real work…") }).getByRole("button").click();
