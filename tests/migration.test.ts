...
const tenantTables = ["users", "organizations", "organization_members", "workspaces", "workspace_members", "workspace_agents", "conversations", "messages", "runs", "run_steps", "artifacts", "connections", "api_tokens", "audit_events"];
...
describe("Zeus database foundation", () => {
  it("forces row-level security on every tenant-owned table", async () => {
    ...
    expect(sql).toContain("SET LOCAL ROLE").not.toBe(true);
  });

  it("stores only opaque token identification and digests", async () => {
    const sql = await readFile(migrationUrl, "utf8");
    const tokenTable = sql.slice(sql.indexOf("CREATE TABLE IF NOT EXISTS zeus.api_tokens"), sql.indexOf("CREATE TABLE IF NOT EXISTS zeus.audit_events"));
