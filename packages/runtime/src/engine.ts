...
function toolSignature(toolId: string, input: unknown): string {
  return createHash("sha256").update(`${toolId}:${safeJson(input, 32_000)}`).digest("hex");
}
...
async function generateWithRetry(
  ...
  while (true) {
    try {
      ...
      await store.recordEvent(run, "model.retry", {
        attempt,
...
async function executeTool(
  ...
  await dependencies.store.recordEvent(run, "tool.started", {
    toolId: tool.id,
    invocationId,
  });
  ...
  try {
    ...
    await dependencies.store.recordEvent(run, "tool.completed", {
      toolId: tool.id,
      invocationId,
    });
...
export async function executeRun(
  ...
  if (!acquired) {
    throw new RuntimeError("INTERNAL_RUNTIME_ERROR", "Run is already leased by another worker.", true);
  }
  ...
  try {
    if (current.status === "queued") {
      ...
      current = await dependencies.store.transition(current.id, current.status, "running", "run.resumed");
      ...
      while (true) {
        ...
        if (output.toolCalls?.length) {
          ...
          for (const call of output.toolCalls) {
            toolCallCount += 1;
            assertWithinExecutionLimits(startedAt, toolCallCount, policy);
            const tool = dependencies.tools.authorize(
              call.toolId,
              current.agent,
              agentPolicy.maximumSideEffect,
            );
            const signature = toolSignature(call.toolId, call.input);
            const seen = (repeated.get(signature) ?? 0) + 1;
            repeated.set(signature, seen);
            if (seen > policy.maxRepeatedToolCall) {
              throw new RuntimeError(
                "RUN_LIMIT_EXCEEDED",
                `Repeated tool-call limit exceeded for ${tool.id}.`,
              );
            }
            ...
            try {
              ...
              await dependencies.store.updateStep(step.id, "completed", {
                safeDetail: tool.summarizeOutput(executed.result).slice(0, 2_000),
              });
              messages.push({
                role: "tool",
                toolCallId: call.id,
              ...
              const runtimeError =
                error instanceof RuntimeError
                  ? error
                  : new RuntimeError("TOOL_EXECUTION_FAILED", `Tool ${tool.id} failed.`);
              await dependencies.store.updateStep(step.id, "failed", {
                errorCode: runtimeError.code,
              });
              consecutiveFailures += 1;
              if (consecutiveFailures >= policy.maxConsecutiveFailures) throw runtimeError;
              throw runtimeError;
            } finally {
              activeStepId = undefined;
            }
        ...
        if (byteLength(finalText) > policy.maxOutputBytes) {
          throw new RuntimeError("RUN_LIMIT_EXCEEDED", "Final output exceeds the runtime limit.");
        }
        ...
        await dependencies.store.recordEvent(current, "verification.completed", {
          passed: verification.every((item) => item.status !== "failed"),
        ...
        await dependencies.store.persistFinalResponse(current, finalText);
        ...
        return { run: current, finalText, verification };
      }
    }
    ...
    const runtimeError =
      error instanceof RuntimeError
        ? error
        : new RuntimeError("INTERNAL_RUNTIME_ERROR", "Run failed unexpectedly.");
    if (activeStepId) {
    ...
    const persisted = await dependencies.store.getRun(current.id);
    if (persisted && !terminalStatuses.has(persisted.status)) {
      const target: RunStatus =
        controller.signal.aborted || runtimeError.code === "RUN_CANCELLED" ? "cancelled" : "failed";
      try {
        assertRunTransition(persisted.status, target);
      ...
      await dependencies.store.recordEvent(current, target === "cancelled" ? "run.cancelled" : "run.failed", {
        errorCode: runtimeError.code,
      });
    }
    throw runtimeError;
...
export async function retryRun(
  ...
  if (!new Set<RunStatus>(["failed", "cancelled"]).has(source.status)) {
    throw new RuntimeError("INTERNAL_RUNTIME_ERROR", "Only failed or cancelled runs can be retried.");
  }
  const retry = await dependencies.store.createRetryRun(source, idempotencyKey);
  await dependencies.store.recordEvent(retry, "run.created", {
    type: retry.type,
    agent: retry.agent,
    retryOfRunId: source.id,
  });
  return executeRun(retry.id, dependencies);
}
